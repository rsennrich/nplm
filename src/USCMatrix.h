...
// simpler and faster compressed representation, as follows:
//  - rows: the theoretical number of rows of the matrix
//  - indexes: for each column, the indexes of the non-zero rows
//  - values: the actual values stored in the matrix
...
class USCMatrix {
 public:
  ...
  USCMatrix(
      Index rows,
      const MatrixBase<Indexes> &indexes,
      const MatrixBase<Values> &values)
      : indexes(indexes), values(values), m_rows(rows) {}
