...
public:
  ...
  multinomial(const std::vector<Count> &counts) : unif_real(0.0, 1.0) {
    estimate(counts);
  }
  ...
  void setup(const std::vector<double>& probs) {
    ...
    for (s_it = S.begin(); s_it != S.end(); ++s_it) {
      ...
      if (std::fabs(q[*s_it] - 1) > tol) {
	      std::cerr << "warning: multinomial: probability differs from one by "
                  << std::fabs(q[*s_it]-1) << std::endl;
      }
      q[*s_it] = 1.0;
    }
