...
class propagator {
 ...
 public:
  ...
  void fProp(const MatrixBase<Derived> &data) {
    if (!pnn->premultiplied) {
      start_timer(0);
	    ...
	    start_timer(1);
	    first_hidden_linear_node.param->fProp(
          input_layer_node.fProp_matrix, first_hidden_linear_node.fProp_matrix);
    } else {
      ...
      start_timer(1);
      first_hidden_linear_node.param->fProp(
          sparse_data, first_hidden_linear_node.fProp_matrix);
    }
    first_hidden_activation_node.param->fProp(
        first_hidden_linear_node.fProp_matrix,
        first_hidden_activation_node.fProp_matrix);
    stop_timer(1);

    start_timer(2);
    second_hidden_linear_node.param->fProp(
        first_hidden_activation_node.fProp_matrix,
        second_hidden_linear_node.fProp_matrix);
	  second_hidden_activation_node.param->fProp(
        second_hidden_linear_node.fProp_matrix,
  ...
  void bProp(const MatrixBase<DerivedIn> &data,
    ...
    start_timer(7);
    output_layer_node.param->bProp(output, output_layer_node.bProp_matrix);
    stop_timer(7);
  ...
  void bProp(
      const MatrixBase<DerivedIn> &data,
    ...
    output_layer_node.param->computeGradient(
        second_hidden_activation_node.fProp_matrix, samples, weights,
        learning_rate, momentum);
    stop_timer(8);
 ...
 private:
  ...
  void bPropRest(
    ...
    // Second hidden layer
    start_timer(9);
    second_hidden_activation_node.param->bProp(
        output_layer_node.bProp_matrix,
        second_hidden_activation_node.bProp_matrix,
        second_hidden_linear_node.fProp_matrix,
        second_hidden_activation_node.fProp_matrix);

    second_hidden_linear_node.param->bProp(
        second_hidden_activation_node.bProp_matrix,
        second_hidden_linear_node.bProp_matrix);
    stop_timer(9);

    start_timer(10);
    second_hidden_linear_node.param->computeGradient(
        second_hidden_activation_node.bProp_matrix,
        first_hidden_activation_node.fProp_matrix,
    ...
    // First hidden layer
	  start_timer(11);
	  first_hidden_activation_node.param->bProp(
        second_hidden_linear_node.bProp_matrix,
				first_hidden_activation_node.bProp_matrix,
				first_hidden_linear_node.fProp_matrix,
				first_hidden_activation_node.fProp_matrix);
	  first_hidden_linear_node.param->bProp(
        first_hidden_activation_node.bProp_matrix,
				first_hidden_linear_node.bProp_matrix);
  	stop_timer(11);

	  start_timer(12);
	  first_hidden_linear_node.param->computeGradient(
        first_hidden_activation_node.bProp_matrix,
				input_layer_node.fProp_matrix, learning_rate, momentum, L2_reg);
  	stop_timer(12);

	  // Input word embeddings
	  start_timer(13);
	  input_layer_node.param->computeGradient(
        first_hidden_linear_node.bProp_matrix, data, learning_rate,
        momentum, L2_reg);
  	stop_timer(13);
  }
};
