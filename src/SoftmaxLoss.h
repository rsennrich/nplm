...
struct SoftmaxLogLoss {
  ...
  void fProp(
      const MatrixBase<DerivedI> &input,
  ...
  void bProp(
      const MatrixBase<DerivedW> &output_words,
      const MatrixBase<DerivedO> &output,
      const MatrixBase<DerivedI> &grad_input_const) {
...
class SoftmaxNCELoss {
 ...
 public:
  SoftmaxNCELoss(const Multinomial &unigram) : unigram(unigram) {}

  template <typename DerivedI, typename DerivedW, typename DerivedO>
  void fProp(
      const MatrixBase<DerivedI> &scores,
  ...
  void bProp(
      const MatrixBase<DerivedO> &probs,
      const MatrixBase<DerivedI> &output_const) {
